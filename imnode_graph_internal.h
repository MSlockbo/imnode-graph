...
struct ImObjectPool
{
    ...
    reverse_iterator rbegin() { return reverse_iterator(*this, Active.size()); }
...
// Connections ---------------------------------------------------------------------------------------------------------
    ...
    void             MakeConnection(const ImPinPtr& a, const ImPinPtr& b);
...
void ImObjectPool<T>::PushToTop(ImGuiID id) // Should always be O(n)
{
	...
	while(idx < Data.Size - 1)
	{
	    ...
	    ImSwap(IdxToID[idx], IdxToID[next_idx]);
		IDToIdx.SetInt(IdxToID[idx], idx);
		idx = next_idx;
	}
